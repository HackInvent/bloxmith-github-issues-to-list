...
(function () {
   ...
   *
   * @param {HTMLElement} root - Mounted block modal root.
   * @returns {HTMLElement[]} Tab buttons controlled by this asset.
   */
  function tabElements(root) {
    return Array.from(root.querySelectorAll("[data-github-issues-to-list-modal-tab]"));
  }

  /**
   * Return block modal panels in DOM order.
   *
   * @param {HTMLElement} root - Mounted block modal root.
   * @returns {HTMLElement[]} Panels associated with modal tabs.
   */
  function panelElements(root) {
    return Array.from(root.querySelectorAll("[data-github-issues-to-list-modal-panel]"));
  }

  /**
   * Activate one tab and hide inactive panels.
   *
   * @param {HTMLElement} root - Mounted block modal root.
   * @param {HTMLElement} tab - Tab element to activate.
   * @param {object} options - Activation options.
   * @param {boolean} options.focus - Whether focus should move to the tab.
   * @returns {void}
   */
  function activateTab(root, tab, { focus = false } = {}) {
    ...
    const tabId = String(tab.dataset.githubIssuesToListTabId || "");
    ...
    for (const panel of panelElements(root)) {
      panel.hidden = String(panel.dataset.githubIssuesToListTabId || "") !== tabId;
    }
    if (focus) {
      tab.focus();
    }
  ...
  /**
   * Move keyboard focus to the next or previous tab.
   *
   * @param {HTMLElement} root - Mounted block modal root.
  ...
  registry.github_issues_to_list = {
    ...previous,

    /**
     * Bind modal tabs while persistence stays on generic block UI fields.
     *
     * @param {HTMLElement} root - Mounted block modal root.
    ...
    mount(root, api, context) {
      ...
      root.addEventListener("click", (event) => {
        const tab = event.target.closest("[data-github-issues-to-list-modal-tab]");
